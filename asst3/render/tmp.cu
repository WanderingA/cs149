...
#define SCAN_BLOCK_DIM                                                         \
  BLOCKSIZE // needed by sharedMemExclusiveScan implementation
...
__global__ void myKernelRenderCircles() {
  ...
  int localThreadIndex = threadIdx.y * blockDim.x + threadIdx.x;
  ...
  float4 pixelColor = *imgPtr; // 读取初始颜色
  ...
  for (int bunch = 0; bunch < numCircles; bunch += BLOCKSIZE) {
    ...
    if (index < numCircles) {
      float3 p = *(float3 *)(&cuConstRendererParams.position[index3]);
      float rad = cuConstRendererParams.radius[index];
      flags[localThreadIndex] =
          circleInBoxConservative(p.x, p.y, rad, boxL, boxR, boxT, boxB);
    } else {
    ...
    sharedMemExclusiveScan(localThreadIndex, flags, prefixSum, Scratch,
                           BLOCKSIZE);
    __syncthreads();
    ...
    int numCirclesInBox =
        prefixSum[BLOCKSIZE - 1] + flags[BLOCKSIZE - 1];
    __syncthreads();
...
__global__ void kernelRenderPixels() {
  ...
  for (int i = 0; i < numCircles; i += BLOCKSIZE) {
    ...
    if (circleIdx < numCircles) {
      ...
      flag[idx] = circleInBoxConservative(
          p.x, p.y, cuConstRendererParams.radius[circleIdx], boxLNorm, boxRNorm,
          boxTNorm, boxBNorm);
    } else {
      flag[idx] = 0;
    }
    ...
    // 进行渲染
    if (pixelX < imageWidth && pixelY < imageHeight) {
      uint num = presum[BLOCKSIZE - 1] +
                 flag[BLOCKSIZE - 1]; // 可能和区域相交的圆的数量
      for (int j = 0; j < num; ++j) {
...
void CudaRenderer::render() {
  ...
  //不要用getImage->width !!! 里面有cudamemcpy函数！
